...
namespace UnityGLTF
{
	public class GLTFSceneImporter
	{
		...
		protected virtual GameObject CreateScene(GLTF.Schema.Scene scene)
		{
		...
		protected virtual GameObject CreateNode(GLTF.Schema.Node node)
		{
		...
		protected virtual MaterialCacheData CreateMaterial(GLTF.Schema.Material def, int materialIndex)
		{
			...
			if (materialIndex < 0 || _assetCache.MaterialCache[materialIndex] == null)
			{
				...
				if (def.AlphaMode == GLTF.Schema.AlphaMode.MASK)
				{
				...
				else if (def.AlphaMode == GLTF.Schema.AlphaMode.BLEND)
				{
		...
		protected virtual UnityEngine.Texture CreateTexture(GLTF.Schema.Texture texture)
		{
			if (_assetCache.TextureCache[texture.Source.Id] == null)
			{
				...
				if (texture.Sampler != null)
				{
					...
					{
						case GLTF.Schema.MinFilterMode.Nearest:
							desiredFilterMode = FilterMode.Point;
							break;
						case GLTF.Schema.MinFilterMode.Linear:
						default:
